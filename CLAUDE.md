...
| `-Wno-discarded-qualifiers` | Suppress the **on-by-default** warning for an implicit conversion that discards a pointee `const` (passing/assigning/initializing/returning a `const T*` where a `T*` is expected; gcc `-Wdiscarded-qualifiers`). An explicit cast is already exempt. Does **not** affect the write-to-const **error** (a constraint violation, not suppressible). The check lives in `IrBuilder` (`CheckQualifierDiscard`, gated by `warnDiscardedQualifiers`); const-correctness is on by default with no `-std=` gate (`const` is C89). |
