...
| `-Wconversion` | Opt-in (off by default, like gcc/clang `-Wconversion` / MSVC C4244): warn to stderr on an implicit integer conversion that **narrows** — a wider value stored into a narrower type (`int` → `unsigned char`) at init / assignment / return. Independent of `-std`/`-pedantic`. dotcc inserts the conversion cast regardless (C# requires it); the flag only controls the *warning*, collected by a `ConversionGate` (mirrors `DialectGate`) and flushed on the same stderr channel. A constant that fits the target is neither cast nor flagged (matches C). Same-width sign changes (`int`↔`uint`) are NOT flagged — that's `-Wsign-conversion` territory, intentionally out of scope. |
...
| C function | `internal static unsafe` method of a top-level `DotCcProgram` class (NOT a top-level local function — class methods can be addressed via `&fn`, stored in function-pointer tables, and referenced across contexts, which Lua's `luaL_Reg`-style code needs; `using static DotCcProgram;` surfaces them by bare name). `-shared` mode uses the parallel `DotCcLib` class. |
