...
`dlfcn.h` row in `C-SUPPORT.md` (POSIX headers). Two of the three pieces now
exist: `-shared` host exports, AND implicit **`-l`/`-L` import mode** (a plugin
TU's undefined, called prototypes bind GOT-style to a named library — no
`dlopen`, no `[LibraryImport]`). What's still missing for a chibi *plugin* host
specifically is **shared runtime state**: plugin TUs must reach the host's heap
and globals (its `sexp` GC arena, the module registry) rather than each carrying
its own runtime — a larger problem than symbol resolution, which import mode and
`dlopen` both solve but neither addresses.
