...
main.c                  #includes geom.h + buildcfg.h, calls them, prints the result
include/buildcfg.h      header reachable ONLY via -I (proves include threading)
CMakeLists.txt          project(... C) + add_executable + target_include_directories/_definitions
...
`target_include_directories()` and `target_compile_definitions()` flow through:
CMake's `<INCLUDES>`/`<DEFINES>` are forwarded to dotcc's compile rule, and
because dotcc is clang-shaped it accepts CMake's default glued spellings
(`-I/abs/dir`, `-DNAME=VAL`) as-is. The demo proves it: `buildcfg.h` lives in
`include/` (found *only* via `-I`) and `SCALE` arrives as `-DSCALE=10` and is
used numerically — hence `tag=cfg-ok scaled=70` in the output. Drop either
flag and the build fails at the `#include` / undefined `SCALE`.
