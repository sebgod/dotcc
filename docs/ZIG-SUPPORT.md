...
| `comptime_int`/`comptime_float` | 🟡 | **A `comptime_int` RETURN type is supported (the comptime-call engine V1, road-to-zig-std G3):** `fn maxInt(comptime T: type) comptime_int` (real `std.math.maxInt` / `minInt` compile from source). The result has no runtime type, so every call is a deferred comptime fold: the instance lowers with a 128-bit carrier, the shared interpreter runs its body once every module has drained (the callee may live in a lazy module) and splices the literal, and the instance itself is dropped from the emitted program. A comptime-only call may nest in another (`maxInt(T) - minInt(T)`). Oracle `comptime_int_calls` + the real-std `Dotcc_matches_zig_std_math_max_min_int_from_source` differential; unit `ZigComptimeCallTests`. A body of comptime `const` bindings and a `return` is EVALUATED when the call is instantiated, so a position that needs the value during lowering has it (an enum member of `maxInt(u64)`); any other body stays a deferred fold. **Leniency:** a folded value that does not fit its sink truncates (`const m: u8 = maxInt(u16);`), as a plain out-of-range literal already does; zig rejects both. **Cuts:** `comptime_int` as a parameter, local or field type, a non-generic `comptime_int` function, `comptime_float` anywhere, a call whose body the immediate path cannot evaluate, in a position that needs its value DURING lowering, and a ROOT unit's enum member calling a function of the same unit (containers register before functions are declared; a lazy module declares on demand) |
