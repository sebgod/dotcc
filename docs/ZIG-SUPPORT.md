...
- **Examples** — `examples/zig-hello`, `examples/zig-extern` (putchar),
  ...
  `signedness`, `child` in a type position, `len`, and `bits` read off the source spelling so
  `@typeInfo(u21)` answers 21 rather than the 32 dotcc lowers `u21` to),
