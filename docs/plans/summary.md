...
Last verified against `main` on **2026-09-03** (`89f36e2`).
...
| [road-to-zig-std](road-to-zig-std.md) | **PARTIAL ~30%** — compiling real upstream zig std from source. ✅ S0 probe, S1 module graph, S2 lazy decl-driven lowering, S4a/b/c comptime-value + captured-`if` folding, several S9 parse bricks (probe at 32.0% of 553 files), G1 (`std.ascii` from real source, oracle-identical), and G4's tentpole (a reified generic carries methods). **Remaining:** S4d (type-position module-graph fallback — the narrow brick that keeps `std.ArrayList` unreachable from source), S3 synthetic `builtin`/`root`, S5–S7 the `@typeInfo` reflection engine, S8 the platform-floor redirect table, the ranked S9 bricks, G2/G3/G5, and G4's blockers 2/3/5. |
