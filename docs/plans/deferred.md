...
| A comptime-built struct holding pointers into comptime arrays | #100 | step 1 landed (a struct-returning `comptime { … }` block is interpreted and spliced with its pointer targets pinned). Left for std.StaticStringMap.initComptime: an array whose extent the block itself computes (`var len_indexes: [self.max_len + 1]u32`) cannot be lowered before the block runs, so the block needs interleaved lowering and evaluation (as `comptime { }` statement blocks get). Two pointers to one comptime array are pinned as separate copies (equal contents, different addresses) |
