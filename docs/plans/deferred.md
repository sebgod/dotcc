...
| The declared width of an `anytype` argument's type (`maxInt(@TypeOf(x))` in std.math.cast) | Refused by the `@typeInfo` fidelity rule: `bits` is answered only from a type SPELLING, and an inferred type carries none (a `u5` value lowers to `byte`, like a `u8`). std.fmt.parseInt now walls here, in `math.cast(u8, c)` | carry the declared width WITH a value's type where it is spelled (a local, a parameter, a capture over a slice of a spelled element type) and into the `anytype` seed, as a `comptime T: type` seed already carries it; answering the lowered width for an unknown spelling is the silently-wrong outcome the rule exists to refuse |
