...
| A GENERIC function called through a file-as-struct type (`w.print(fmt, args)`) | Loud: "called through its file-as-struct type". A file-as-struct's methods ARE its module's top-level functions, so a `comptime`/`anytype` one is a template whose placeholder signature must not be bound | instantiate through `TryResolveExportedGenericInstance` with the receiver as the first argument; for a file-as-struct this is the generic-METHOD wall reached through free functions, which the cross-module generic machinery already half-covers. **bufPrint wall 2** |
| A pointer field to a container that could not lower fails the struct holding it | A lazy module's container that fails to register is withdrawn and raises at its first reference (zig's analyse-on-reference rule). But a FIELD typed `*const T` is a reference too, so the failure propagates up every struct that points at it, although zig never needs a pointee's layout to size a pointer. bufPrint hits this today: `Writer.VTable.sendFile` takes a `*File.Reader`, `File.Reader` holds a `File`, and `File.handle: Handle` is a platform alias dotcc does not resolve | lower a pointer to an unregistered container as an OPAQUE pointer that raises the recorded failure on any dereference or field access, so only a real USE of the pointee fails. **bufPrint's current wall** |
