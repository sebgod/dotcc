...
| A write through a pointer to a `const` (`const p = &y; p.* = 2;`, `if (c) \|*v\| v.* = 2;` on a const) | #95 | zig: "cannot assign to constant". dotcc types `&` of a const binding as a plain `*T` and does not check writes through a `*const T`, so it accepts both and mutates the const |
