...
| `@typeInfo(T).<kind>.bits` through a type ALIAS or a `comptime T: type` param | S5a | the DECLARED width is only in the source spelling: dotcc widens `uN`/`iN` to the smallest standard width (`u21` → 32-bit `uint`), and a type binding does not carry the original. A direct `@typeInfo(u21).int.bits` folds to 21; through a binding it is a loud cut, because answering the lowered 32 would silently disagree with zig. **Fix:** carry the declared width on the type — which C23 `_BitInt(N)` wants anyway — rather than a side map keyed by alias name (the shadow/restore surface of `_typeAliases` makes a parallel map easy to get subtly wrong) |
| `@typeInfo(T).<aggregate>.fields` / `.decls` | S5a | a field/decl list is a comptime SLICE of comptime aggregates — S5's aggregate value domain plus S6's `inline for` over it. The TAG of every aggregate kind already dispatches, so a `switch (@typeInfo(T))` over a struct/enum/union works; only the member lists are cut |
