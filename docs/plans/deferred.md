...
| `inline for` over a `@typeInfo` member list | S6 | 95 uses in 37 files, and the lists it iterates all exist now (S5c) — this is the highest-value remaining reflection brick, not a cut for a reason |
