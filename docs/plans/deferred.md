...
| `if (opt) \|*v\|` pointer capture on an `if` | #94 | std.enums.EnumMap.init (`if (@field(init_values, tag)) \|*v\|`) does not parse. A value optional (`Nullable<T>`) has no addressable payload, so the capture needs a payload temp written back after the body |
