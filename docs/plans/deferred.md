...
| The `@EnumLiteral()` builtin as a TYPE spelling (std.Options' `log_scope_levels` fields) | #113 | a bare enum literal's value type is modeled (`CType.EnumLiteral`, one singleton per name), but spelling the type itself names no member, so a field or parameter declared `@EnumLiteral()` is not |
| An enum member valued by the ROOT unit's own function (`b = maxOf(u16)` in a single file) | #123 | the root unit registers containers (pass 0) before it declares its functions (pass 1); a lazily prepared module (std) records its function decls first, so std.Io.Limit's `math.maxInt(usize)` works, but the root unit would need on-demand declaration that passes 1 and 2 then skip |
| `{any}` of a tuple nested in the argument tuple (`.{ .{ @as(u8, 4) } }`) | #121 | a tuple argument's per-element declared widths are tracked one level deep, so `@typeInfo(@TypeOf(x)).int.bits` of an element of the inner tuple is not known; `{any}` of a struct field or a top-level argument is |
...
| `@typeInfo(T).pointer.size` of a raw pointer no spelling classifies | S5a, #119 | dotcc lowers `*T`, `[*]T` and `[*c]T` to one C pointer, so the SIZE class (`.one`/`.many`/`.c`) rides the source spelling (task #119: `&x`, a typed local, a parameter, an alias, an `anytype` argument). A pointer returned by a call or read from a struct field carries none, and reporting `.one` for it would be wrong for `[*]T`, so it stays loud. Recoverable by recording a function's spelled return class and a field's, as `bits` does, if it earns demand |
