...
| A `comptime_int` argument beyond 64 bits (sort's `math.log2(math.maxInt(usize) + 1)`) | T5 | a comptime value seed is a `long`, so `2^64` does not bind to an `anytype` as a comptime_int; the call then falls to the runtime path. Widening the seed domain to `Int128` touches every seed consumer (the mangle, `ComptimeVarLit`, the optional seeds) |
