...
| A comptime-built struct holding pointers into comptime arrays | #100 | steps 1-2 landed (a struct-returning `comptime { … }` block is interpreted statement by statement and spliced with its pointer targets pinned; tuples evaluate and iterate at comptime). Left for std.StaticStringMap.initComptime: its `comptime kvs_list: anytype` tuple argument has no comptime value inside the instance (the instance is keyed by type only), then a comptime sort and splicing arrays of strings. Two pointers to one comptime array are pinned as separate copies (equal contents, different addresses) |
