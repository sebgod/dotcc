...
| A comptime-built struct holding pointers into comptime arrays | #100 | std.StaticStringMap.initComptime's inline `comptime { var self = Self{}; …; self.kvs = &.{ .keys = &final_keys, … }; return self; }` is lowered as runtime code (only a slice-returning comptime block runs in the interpreter), and splicing a struct that points at comptime arrays needs pinned statics for them |
