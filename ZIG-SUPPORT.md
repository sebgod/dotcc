...
everything else errors clearly. Legend: ✅
...
| `[N:s]T` sentinel array (local) | ✅ | sentinel-terminated array (Milestone O, part 4; non-zero sentinel in Milestone Z). Reserves **N+1** elements of storage: the trailing slot (index `N`) holds the sentinel `s`, the logical length stays `N` — so a `[N:0]u8` literal is a valid NUL-terminated C string with no hand-written terminator, and `b[N]` reads the sentinel back. Lowers to the same `stackalloc` an `[N]T` local uses, grown by one slot. A **zero** sentinel rides C#'s zero-fill (`undefined` reserves `N+1` zeroed; a literal lays down its `N` elements + a trailing `0`); a **non-zero** sentinel is materialized — appended to a literal (`stackalloc i32[]{ …, s }`) or written into the trailing slot for `undefined` (`b[N] = s;`, since zero-fill would leave 0). The symbol's type is the N-element `CType.Array`, so indexing/slicing behave like `[N]T`. **Cut:** a **global** `[N:s]T` (the pinned store has no N+1 hook yet — declare it as a local; rejected loudly). Note an `undefined` array's sentinel is dotcc-defined (real zig leaves it undefined) |
...
| `std.mem.eql(T, a, b)` / `std.mem.copyForwards(T, dest, src)` | ✅ | curated `std.mem` slice helpers → the runtime `ZigMem` class (`DotCC.Libc/ZigMem.cs`, auto-spliced like `ZigAlloc`). `eql` = equal length AND identical contents (byte-wise, = element equality for the scalar element types it's used with); `copyForwards` = copy `src.len` elements front-to-back. Slice args coerce a `&array` (`*[N]T`) to a `[]T`/`[]const T` fat pointer. Any OTHER `std.mem.*` member errors clearly ("not modeled yet") — `std` is a curated-paths resolver, not a general model. **Cut:** `std.mem.span` / `std.mem.zeroes` (a later slice) |
...
| `[N:s]T` non-zero sentinel array | ✅ | a non-zero sentinel `[N:s]T` array (Milestone Z) materializes `s` in the trailing slot (literal-appended / written for `undefined`). `[*:s]T`/`[:s]T` pointer & slice sentinels are type-level only (erased; the value is unused without the cut auto-scan). **Deferred:** a **global** `[N:s]T` array |
...
- **Examples** — `examples/zig-hello`, `examples/zig-extern` (putchar),
  ...
  `std.mem.copyForwards`, `@memset`/`@memcpy`, plus the `&array` → `[]const u8` slice coercion).
