...
`std.ArrayList(T)` as W0, in-function `const P = struct { … };` decls as W2, and
...
W3a**. Type params (`comptime T: type`), `anytype`, and type-returning functions are
...
**Planned — the arc is underway (W0/W1/W2/W3a shipped), the rest are loud errors:** the
...
decls (**W2**, the reify-a-struct primitive), and **generic functions via `comptime`
VALUE params** (**W3a** — call-site monomorphization: one specialized, memoized body
per resolved value, with comptime-if folding so recursive generics terminate) — all in
the Types table. Still to come: comptime TYPE params + type-returning functions
(**W3b/W4** — the core generic case, where the signature lowers per-instantiation) /
