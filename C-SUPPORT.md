...
**C99**
- 🟡 Compound literals — `(struct S){…}` ✅ (struct/union, any position); `(int){5}` ✅ (scalar → cast, any position); `(int[]){…}` ✅ (array, initializer position → stackalloc; other positions can't escape a stackalloc). See the Types table.
