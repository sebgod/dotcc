...
| **libc (20 headers)** | ✅ Near-complete | stdio (incl. real `FILE*` file I/O), stdlib, string, math + tgmath, ctype, time (scalar + `struct tm` calendar + `_r`), errno, assert, complex, inttypes, iso646, stdint, limits, float, stddef, stdbool, **stdarg** | 🟡 threads (subset), setjmp (`if/else` shapes) · 🚫 signal, Annex K |
...
| `const T` / `volatile T` qualifiers | ✅ | **Stripped at the token level** by `QualifierStripper` (a `RewritingTokenStream` after `MacroExpander`, before `TypeNameRewriter`): every `const`/`volatile` token is deleted before the parser sees it. dotcc has no C# representation for either (`const char *p` lowers exactly like `char *p`), so dropping them was always the semantics — doing it as a token rewrite (rather than via the old `TypeSpec`/`ResolveTypeSpec` drop, which only covered built-in-base specifier lists) is what lets a qualifier prefix a **typedef-name** (`const TValue *`), a **struct/union/enum tag** (`const struct X`), an **east-const** declarator (`T const *`), and arbitrary **multi-qualifier runs** (`const volatile T`) — none of which the grammar could parse, and which can't be added without an unavoidable LALR reduce/reduce conflict (at the first `const` the parser can't tell a built-in spec list from a typedef-name base). Covers leading, intermixed, and post-`*` positions uniformly. `restrict` is intentionally NOT stripped (pointer-only; handled by the production below). Both C90, no dialect gate. Fixtures `const-qualifiers/`, `c-idioms/`. **⚠️ Limitation — no const-correctness:** because the qualifier is erased, dotcc does **not** diagnose a write through a `const` lvalue (e.g. `*p = 5;` where `p` is `const int *`); a compiler that *checks* const-correctness would reject it. dotcc transpiles already-valid C (Lua, etc., which gcc/clang have already const-checked), so this is acceptable today — but a real const-correctness pass (carry const on the `CType` layer, error at mutating sinks) is a wanted future feature. |
| `restrict` (C99) / `int * const` (pointer qualifiers after `*`) | ✅ | `restrict` after the pointer star — `int *restrict p` — parses and is **dropped** (`TypePtrQualRestrict`); dotcc has no aliasing model, so `restrict` carries no semantics today (a future optimization could map a `restrict` parameter to a by-ref / no-alias hint). `int * const p` / `int * volatile p` are now handled upstream by `QualifierStripper` (the qualifier token is gone before the parser runs); the `TypePtrQualConst`/`Volatile` productions remain in the grammar but are inert. GNU `__restrict` / `__restrict__` lex to the same token. Fixture `restrict-qualifier/`. |
| Function pointer `T (*fp)(args)` | ✅ | Two forms. **typedef'd**: `typedef int (*Name)(args);` → `using unsafe Name = delegate*<args, int>;`. **Standalone local declarator** (the famously-tricky embedded-name form): `int (*op)(int, int) [= E];` → a `delegate*<int, int, int>` local — conflict-free because the `( * ID )` shape is distinct from every `Type ID` declarator. A bare function-name initializer (`= add`, which C decays to a pointer) gets the `&` C# requires; `&add` passes through; `&function_name` is C#'s `&LocalFunction`. Calling `op(x, y)` works (C# function-pointer invoke, unsafe). **Standalone parameter**: `int apply(int (*op)(int,int), …)` → a `delegate*<…>` parameter (the pointed-to type's own params stay off the enclosing signature). A **bare function name as a call argument** auto-decays to `&fn` (C's function-to-pointer conversion), so `apply(add, …)` and `qsort(…, compare)` pass naturally. Fixtures `fnptr-sort/` (typedef'd `Comparator`), `fnptr-local/` (standalone local), `fnptr-param/` (standalone param). **Gap:** a bare function name in an *assignment* (`op = add;`) still needs explicit `&` (no lvalue-param-type tracking). |
