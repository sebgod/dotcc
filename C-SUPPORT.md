...
| Compound literals `(struct S){…}` (C99) | 🟡 | `(Type){ init }` — an unnamed object usable in any expression position (initializer, assignment, call argument, `return`). **Struct/union types** lower to a C# object-creation `new Type { field = val, … }` — positional (`(struct Point){3,4}`, field names from `_structFields`) and designated (`(Color){.r=255}`) forms both work. Two new `Unary` productions (`compoundLit` / `compoundLitDesignated`) next to the cast rule; conflict-free because the `{` lookahead after `( Type )` isn't in FIRST(Unary). Gated C99 under `-pedantic`. Fixture `compound-literals/` (gcc-oracle-validated). **Gap:** array/scalar compound literals (`(int[]){1,2,3}`, `(int){5}`) aren't supported — the `Type` grammar has no abstract array declarator, and a non-struct `Type` fails loudly with a clear message rather than miscompiling. |
