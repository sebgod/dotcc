...
| Anonymous structs/unions | C11 | 🟡 | Unnamed aggregate members whose fields are *promoted* into the enclosing aggregate. **Anonymous `struct` member ✅**: for a struct (sequential, no overlap) promotion is an inline — dotcc emits the inner fields directly into the parent (`struct O { int t; struct { int x, y; }; }` → `O` has fields `t, x, y`), so `o.x` resolves to a real field with C-matching layout and needs no access rewrite. C11-gated. Fixture `anon-struct-member/`. **Anonymous `union` member ❌** (next): fields must *overlap*, so it needs a generated nested explicit-layout type + type-aware access rewrite (`o.i` → `o.__anon.i`) — see the union row. |
...
- 🟡 Anonymous `struct` member ✅ (fields inlined/promoted); anonymous `union` member ⛔ (overlap needs nested type + access rewrite). See the Beyond-C99 table.
