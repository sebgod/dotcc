...
| **Declarations** | ✅ Near-complete | function defs + prototypes, **variadic functions (`...` + `<stdarg.h>`)**, multi-declarators, initializer lists (positional / designated / empty / nested), `inline`, `_Noreturn`, `static`/`extern`/`auto`, mixed decls-and-statements | 🟡 `register` (dropped) · ❌ file-scope array declarators, `_Thread_local`, `_Alignas`, `constexpr`, `[[attributes]]` |
