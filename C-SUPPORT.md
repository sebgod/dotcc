...
- **Dialect flags** — `-std=c99`, `-pedantic`, and `-pedantic-errors` are recognized (`-pedantic`/`-pedantic-errors` drive the dialect-rejection gate). `-Wall` / `-W*` warning flags and other unknown options still aren't accepted; a `./configure` passing them would error out. Fix is mechanical: accept-and-ignore unknown flags in `System.CommandLine`, log a one-line warning at most.
