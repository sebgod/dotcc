...
| **`-MD`/`-MMD` dependency files** | ❌ | **Highest-value next step.** dotcc emits no `.d` depfiles, so CMake/Ninja/Make can't track header→TU dependencies → no correct *incremental* rebuilds (edit a header, dependent TUs aren't recompiled). Self-contained fix: when `-MD -MF <file>` is given, write a Make-format rule listing the TU + every `#include`d header. The preprocessor already resolves the full include set, so the data is in hand — this is plumbing, not new analysis. |
